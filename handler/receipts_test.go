...
func TestHandlerInsert(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			id: 2, useCase: "Negative case: invalid body reader",
			path:             "/v1/receipts/process",
			reqBody:          io.NopCloser(&CustomReader{}),
			statusCode:       400,
			expectedResponse: "read error",
			mockCall:         nil,
		},
		{
			id: 3, useCase: "Negative case: invalid body",
			path:             "/v1/receipts/process",
			reqBody:          bytes.NewBuffer([]byte(`{""}`)),
			statusCode:       400,
		...
		{
			id: 4, useCase: "Negative case: empty body",
			path:             "/v1/receipts/process",
			reqBody:          bytes.NewBuffer([]byte(`{}`)),
			statusCode:       400,
		...
		{
			id: 5, useCase: "Positive case: valid body",
			path:             "/v1/receipts/process",
			reqBody:          bytes.NewBuffer([]byte(`{"retailer": "Target", "purchaseDate": "2022-01-01", "purchaseTime": "13:01", "items": [{"shortDescription": "Mountain Dew 12PK", "price": "5.00"}], "total": "5.00"}`)),
			statusCode:       201,
			expectedResponse: "",
			mockCall: receiptService.EXPECT().Insert(&model.Receipt{
	...
	for _, tc := range testCases {
		w := httptest.NewRecorder()
		r := httptest.NewRequest("POST", tc.path, tc.reqBody)
...
func TestHandlerHealth(t *testing.T) {
	...
	testCases := []struct {
		...
		{id: 1, useCase: "Positive case: Healthy Service", statusCode: 200, expectedResponse: "\"{'health': 'ok'}\""},
	}
...
// CustomReader simulates an error when Read is called. Read is executed by io.ReadAll()
type CustomReader struct{}

func (cr *CustomReader) Read(p []byte) (n int, err error) {
	return 0, er.New("read error")
}
